  ...
  <transition name="modal" appear>
    <div class="modal modal-overlay" @click.self="$emit('close')">
      <div class="modal-window">
        <div v-if="anyRequest" class="modal-content">
          <h3 class="after-attack-text">すでにアタック済みの募集です</h3>

          <button class="select-btn team-create-btn hover-opacity">
            <a href="/" class="modal-select-link">トップに戻る</a>
          </button>
        </div>
        <div v-else class="modal-content">
          ...
          <div class="attack-select-btn-wrapper" style="margin-bottom:32px;">
            ...
            <button class="select-btn team-create-btn hover-opacity">
              <a href="/attack_groups/new" class="modal-select-link">新規チーム作成</a>
            </button>
          </div>
...
<script>
export default {
  props: ['hasAnyAttackGropups', 'anyRequest'],
  methods: {
    showFriendList() {
